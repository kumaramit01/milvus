...
func runExpressionTest(ctx context.Context, client *milvusclient.Client, testCase TestCase) error {
	...
	// Create expression reranker function with proper input fields
	...
	searchResult, err := client.Search(ctx, milvusclient.NewSearchOption(
		...
		// Explicitly mark which reranker runs where; keep proxy-level legacy API name
		WithQueryNodeReranker(exprRerankerFunction).
